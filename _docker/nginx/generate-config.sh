...
if [ -f "$ENV_FILE" ]; then
    for var in APP_URL SSL_CERT_FILE SSL_KEY_FILE; do
...
if [ -f "$TEMPLATE_DIR/server.conf.template" ]; then
    ...
    envsubst '$DOMAIN_NAME $APP_URL $SSL_CERT_FILE $SSL_KEY_FILE' < "$TEMPLATE_DIR/server.conf.template" > "$OUTPUT_DIR/server.conf"
